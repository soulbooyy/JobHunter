...
expected = {"COM": 37, "WSP": 7, "MAE": 18, "STO": 20, "PRF": 23}
found: list[str] = []
for path in contracts.rglob("*.md"):
    found.extend(re.findall(r"^\*\*((?:COM|WSP|MAE|STO|PRF)-\d{3})\.\*\*", path.read_text(), re.M))
assert len(found) == len(set(found)) == sum(expected.values())
...
for requirement in re.findall(r"\b(?:COM|WSP|MAE|STO|PRF)-\d{3}\b", trace):
...
print(f"{len(found)} unique requirements; traceability IDs and linked files resolve.")
