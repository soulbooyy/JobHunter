...
There is one global versioned PreferenceSet. Before initial automatic collection, the user configures search intent; unset dimensions impose no restriction. Every configured preference is hard. The product invents neither defaults nor soft-preference weights. Target roles, cities, or companies mentioned in examples do not limit supported acquisition scope or establish tested quality.

QuickScreen compares Job/source metadata against exact Preferences and screening policy. Only a definite conflict rejects. Missing information remains uncertain, not a rejection or proof of suitability. QuickScreen does not evaluate career capabilities and consumes no CandidateProfile, Evidence, Resume, RequirementSet, or duplicated screening snapshot.

Changing saved Preferences creates a new version and immediately updates local filtering. Temporary keywords, sorting, or view filters do not edit Preferences. There are no named SearchProfiles or parallel long-lived search configurations in v1.

**Sources:** Q45, Q55–Q56, S17.1. Q56 removes Q8's screening snapshot and, together with Q110, supersedes Q10's proposed QuickScreen use of RequirementSet.

### 4.3 Local browsing and explicit collection

Normal Job Pool browsing uses persistent local data. Platform access occurs for the agreed initial collection after Preferences setup or an explicit collection/refresh action. Opening the pool or changing Preferences does not fetch the platform again.

BOSS collection screens list metadata first. Definite rejection stops detail access and creates no local Job. Rejected title, company, URL, and JD content must not be retained as a recoverable rejected-listing dataset. Passing and uncertain entries proceed to full-detail acquisition; only complete validated results are admitted. Collection audit preserves operational usage, aggregate rejection reasons, failures, and risk/stop reasons without becoming another Job store. Collection retains JD but does not initiate Requirement parsing.

An active collection keeps the Preferences with which it started. If the user changes Preferences, the UI explains that the active Run still uses the earlier version and permits explicit stop. Current local filtering uses the latest version immediately. Stop prevents subsequent accesses and retains committed partial results; it does not roll them back, delete newly unmatched Jobs, restart collection, or launch another collection automatically.

**Sources:** Q49, Q55, Q110, Q161, S9.1. Q161 controls active-Run versus current-view semantics.
...
Selection freezes the known requested inputs. Missing dependencies are prepared before the complete per-analysis request is frozen. Before dispatch, revalidate applicable QuickScreen non-rejection, source/dependency compatibility, current-fact eligibility, and permissions. Inputs never silently follow newer versions during preparation or execution; incompatible changes stop the affected task and require a new request against current inputs. Failed final validation after a model call preserves actual usage and audit without publishing an invalid formal analysis.
